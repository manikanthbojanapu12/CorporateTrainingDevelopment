...
(function redirectButtonActionsTo404() {
  document.addEventListener('click', (event) => {
    const action = event.target.closest('a.btn, button.btn, .qa-btn, .action-confirm-btn');
    const isFormSubmit = action?.matches('button[type="submit"], input[type="submit"]') && action.form;
    if (!action || isFormSubmit || action.id === 'confirmLogout' || action.closest('.role-card') || action.hasAttribute('data-allow-navigation') || action.getAttribute('href') === 'login.html') return;
...
(function enforceAlphabetNameFields() {
  const bind = () => {
    document.querySelectorAll('#contactName, #bookOwner, #signInName, #regFirstName, #regLastName').forEach((input) => {
      input.setAttribute('pattern', '[A-Za-z ]+');
      input.setAttribute('title', 'Use alphabets and spaces only');
...
(function initExpertSocialReveal() {
  const bind = () => {
    const cards = document.querySelectorAll('.ctd-expert-card');
    if (!cards.length) return;

    cards.forEach((card) => {
      if (card.dataset.expertBound === 'true') return;
      card.dataset.expertBound = 'true';
      card.setAttribute('tabindex', '0');

      card.addEventListener('click', (event) => {
        if (event.target.closest('a')) return;
        cards.forEach((other) => {
          if (other !== card) other.classList.remove('is-social-open');
        });
        card.classList.toggle('is-social-open');
      });
    ...
    document.addEventListener('click', (event) => {
      if (event.target.closest('.ctd-expert-card')) return;
      cards.forEach((card) => card.classList.remove('is-social-open'));
    });
  };
...
(function initCursor() {
  ...
  const spider = { x: pointer.x, y: pointer.y };
  let pulse = 0;
  ...
  function drawLeg(cx, cy, side, index, stride) {
    const yOffset = -7 + index * 5;
    const reach = 12 + index * 3;
    const kneeX = cx + side * (reach + Math.sin(stride + index) * 2);
    const kneeY = cy + yOffset + Math.cos(stride + index) * 2;
    const footX = cx + side * (22 + index * 3);
    const footY = cy + yOffset + 9 + Math.sin(stride * 1.3 + index) * 4;
    ctx.beginPath();
    ctx.moveTo(cx + side * 5, cy + yOffset);
  ...
  function drawSpider() {
    ...
    spider.x += (pointer.x - spider.x) * 0.28;
    spider.y += (pointer.y - spider.y) * 0.28;
    pulse += 0.18;
...
(function initAquarium() {
  ...
  bgImg.src = 'assets/training-hero.webp';
  let bgLoaded = false;
  ...
  function initWebGL() {
    ...
    const vs = compileShader(gl.VSupportTEX_SHADSupport, vsSource);
    const fs = compileShader(gl.FRAGMENT_SHADSupport, fsSource);
    if (!vs || !fs) {
      isWebGLFallback = true;
      rippleCanvas.style.display = 'none';
      return;
    ...
    gl.bindBuffer(gl.ARRAY_BUFFSupport, positionBuffer);
    gl.bufferData(gl.ARRAY_BUFFSupport, new Float32Array([
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTSupport, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTSupport, gl.LINEAR);
  ...
  function drawWebGL() {
    ...
    try {
      ...
      gl.clear(gl.COLOR_BUFFSupport_BIT);
  ...
  // Start after first paint so CSS aquarium + hero training visual show instantly
  let started = false;
  const start = () => {
    if (started) return;
    started = true;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('agreeTerms')?.addEventListener('change', (event) => {
    if (event.target.checked) window.location.href = '404.html';
  });
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', (e) => {
      ...
      const company = document.getElementById('regLearnerName') || document.getElementById('regClientName');
      ...
      if (!company?.value.trim()) { showErr(company, companyErr, 'Company / team name is required'); ok = false; }
...
window.StacklySelect = {
  ...
  enhance(select) {
    ...
    select.addEventListener('change', syncClientel);
    syncClientel();
  },
...
window.StacklyDate = {
  ...
  enhance(input) {
    ...
    const syncClientel = () => {
      labelEl.textContent = fmtClientel(parseValue());
    };
    ...
    input.addEventListener('change', syncClientel);
    syncClientel();
  },
...
function initVetDashboard() {
  ...
  const defaultName = isAdmin ? 'Coach Rajesh Kumar' : 'Anbarasan K';
